...
// Explicitly assigned to window to guarantee global scope
// regardless of how the browser/CDN processes this file.
// ════════════════════════════════════════
...
window.scrapeHeroBlocks = function() {
  const blocks = [];
  const defs = [
...
window.createGameCanvas = function() {
  ...
  const reloadH = (e) => { if ((e.metaKey || e.ctrlKey) && e.key === 'r') cleanup(); };
...
window.drawBlock = function(ctx, b, accent) {
  if (!b.alive) return;
  const a = b.hp / b.maxHp;
  const col = b.color;
  ctx.globalAlpha = a * 0.2; ctx.fillStyle = col; ctx.fillRect(b.x, b.y, b.w, b.h);
  ctx.globalAlpha = a * 0.7; ctx.strokeStyle = col; ctx.lineWidth = 1;
...
window.hitBlock = function(b, x, y, r) {
  return b.alive && x + r > b.x && x - r < b.x + b.w && y + r > b.y && y - r < b.y + b.h;
};

window.damageBlock = function(b) {
...
window.showGameOver = function(ctx, W, H, msg, score, cleanup, relaunch) {
